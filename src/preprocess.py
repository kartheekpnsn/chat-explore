...
class Preprocess:
        ...
        for idx, line in enumerate(self.data):
            ...
            if len(split_part) <= 1:
                ...
                if len(split_part) < 2:
                    ...
                    split_part = split_part.split(" ")
                    if len(split_part) < 2:
                        if self.is_valid_format(split_part[0]):
    ...
    def prepare_df(self):
        ...
        self.pd_data["Timestamp"] = pd.to_datetime(
            self.pd_data["Timestamp"].str.lower(), format="%d/%m/%y, %H:%M"
        )
        # self.pd_data['Timestamp'] = pd.to_datetime(self.pd_data['Timestamp'].str.lower(), format='%d/%m/%y, %I:%M %p')
