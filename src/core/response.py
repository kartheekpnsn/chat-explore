...
class Response:
    ...
    def __init__(self, data: Optional[pd.DataFrame] = None, logger=None):
        if logger is None:
            self.logger = Logger(log_flag=True, log_file="response", log_path="../logs/")
        else:
            self.logger = logger
        self.data = data
    ...
    def compute(self) -> dict:
        ...
        result = {u: {"median_response_min": None, "p75_response_min": None, "n_responses": 0}
                  for u in users}
    ...
    def get_the_longest_conversation_date(self) -> Optional[str]:
        ...
        grp1 = (
            grouped.groupby("Date")["Response (Min)"].mean().reset_index()
        )
