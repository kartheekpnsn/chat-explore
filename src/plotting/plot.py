...
class Plot:
    ...
    def plot_date_n_msgs(self):
        ...
        full_date_df = Plot.formulate_date_df(
            start_date=min(date_n_msgs["Date"]), end_date=max(date_n_msgs["Date"])
        )
    ...
    def plot_date_n_emojis(self):
        ...
        full_date_df = Plot.formulate_date_df(
            start_date=min(date_n_emojis["Date"]), end_date=max(date_n_emojis["Date"])
        )
    ...
    def plot_domain_counts(self):
        ...
        if domain_counts.shape[0] > 0:
            ...
            g.set_xlabel(
                "Domain of the link shared", size=12, fontdict={"fontweight": "bold"}
            )
        plt.savefig(
