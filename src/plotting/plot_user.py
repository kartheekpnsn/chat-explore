...
class PlotUser:
    ...
    def __init__(self, store=None, user_idx=1, user_color="#2ecc71", save_path="plots/"):
        self.store = store
    ...
    def plot_top_k_ngrams(self, n_grams=1, k=10):
        ...
        plt.savefig(f"{self.save_path}/top_{k}_{n_grams}words_u{self.user_idx}.png",
    ...
    def plot_word_cloud(self, n_grams=1):
        ...
        plt.savefig(f"{self.save_path}/word_cloud_{n_grams}words_u{self.user_idx}.png",
                    dpi=300, bbox_inches='tight', pad_inches=0)
