...
class PlotProgression:
    ...
    def _plot_monthly(self, df, y, filename):
        ...
        for each_user in np.unique(df["User"]):
            ...
            g = sns.lineplot(
                ...
                y=y,
                data=subset,
                color=self.color_map[each_user],
                label=each_user,
                sort=False,
                ax=ax,
            )
    ...
    def plot_monthly_emoji_progression(self):
        ...
        self._plot_monthly(
            df, "Avg. # of Emojis (per Msg)", "monthly_emoji_progression"
        )

    def plot_first_text_progression(self):
        df = self.store.load_df("monthly_first_text_ct")
        self._plot_monthly(
            df, "No. of times user texted first", "monthly_first_text_ct"
        )
    ...
    def plot_monthly_response_time_progression(self):
        ...
        self._plot_monthly(
            df, "Avg. Response (Min)", "monthly_response_time_progression"
        )
