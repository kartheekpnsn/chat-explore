...
class HTMLImages:
    ...
    def populate_word_cloud(self):
        ...
        self.word_cloud_1words_u2 = HTMLImages.encode_decode_img(
            f"{self.save_path}/word_cloud_1words_u2.png"
        )
        self.word_cloud_2words_u1 = HTMLImages.encode_decode_img(
            f"{self.save_path}/word_cloud_2words_u1.png"
        )
        self.word_cloud_2words_u2 = HTMLImages.encode_decode_img(
            f"{self.save_path}/word_cloud_2words_u2.png"
        )
        self.word_cloud_3words_u1 = HTMLImages.encode_decode_img(
            f"{self.save_path}/word_cloud_3words_u1.png"
        )
        self.word_cloud_3words_u2 = HTMLImages.encode_decode_img(
            f"{self.save_path}/word_cloud_3words_u2.png"
        )
        return self
    ...
    def populate_rest_images(self):
        ...
        self.date_n_emojis = HTMLImages.encode_decode_img(
            f"{self.save_path}/date_n_emojis.png"
        )
        self.weekly_pattern = HTMLImages.encode_decode_img(
            f"{self.save_path}/weekday_n_msgs.png"
        )
        self.hourly_pattern = HTMLImages.encode_decode_img(
            f"{self.save_path}/hour_n_msgs.png"
        )
        self.monthly_msg_pattern = HTMLImages.encode_decode_img(
            f"{self.save_path}/monthly_msg_progression.png"
        )
        self.monthly_word_pattern = HTMLImages.encode_decode_img(
            f"{self.save_path}/monthly_word_progression.png"
        )
        self.monthly_emoji_pattern = HTMLImages.encode_decode_img(
            f"{self.save_path}/monthly_emoji_progression.png"
        )
        self.monthly_response_pattern = HTMLImages.encode_decode_img(
            f"{self.save_path}/monthly_response_time_progression.png"
        )
        self.monthly_first_text_ct = HTMLImages.encode_decode_img(
            f"{self.save_path}/monthly_first_text_ct.png"
        )
        self.monthly_avg_polarity = HTMLImages.encode_decode_img(
            f"{self.save_path}/monthly_avg_polarity.png"
        )
        self.domain_counts = HTMLImages.encode_decode_img(
            f"{self.save_path}/domain_counts.png"
        )
        return self
...
class HTMLStats:
    ...
    def populate_tops(self):
        ...
        self.active_date_duration_overall = self.overall.top_active_day[
            "Duration (Min)"
        ]
        # Active Day Emojis
        ...
        self.lc_date_duration_overall = self.overall.longest_conversation_day[
            "Duration (Min)"
        ]
...
class HTML:
    ...
    def populate_members(self):
        ...
        self.html_stats = HTMLStats(
            user1=self.user1, user2=self.user2, overall=self.overall, logger=self.logger
        )
        self.html_images = HTMLImages(save_path=self.save_path, logger=self.logger)

        self.html_images.populate_word_cloud().populate_rest_images()
        self.html_stats.populate_names().populate_title().populate_totals().populate_averages().populate_sentiments().populate_tops().populate_response().populate_emoji_ranks()
        return self
    ...
    def populate_html_txt(self):
        ...
        self.html_txt = self.html_txt.replace(
            "{sup_title}", f"{self.html_stats.sup_title}"
        )
        # User names
        self.html_txt = self.html_txt.replace(
            "{user1}", f"{self.html_stats.user1_name}"
        )
        self.html_txt = self.html_txt.replace(
            "{user2}", f"{self.html_stats.user2_name}"
        )
        # Messages Sent
        self.html_txt = self.html_txt.replace(
            "{n_msgs_u1}", f"{self.html_stats.n_msgs_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_msgs_u2}", f"{self.html_stats.n_msgs_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_msgs_overall}", f"{self.html_stats.n_msgs_overall}"
        )
        # Words Used
        self.html_txt = self.html_txt.replace(
            "{n_words_u1}", f"{self.html_stats.n_words_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_words_u2}", f"{self.html_stats.n_words_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_words_overall}", f"{self.html_stats.n_words_overall}"
        )
        # Unique words used
        self.html_txt = self.html_txt.replace(
            "{n_unique_words_u1}", f"{self.html_stats.n_unique_words_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_unique_words_u2}", f"{self.html_stats.n_unique_words_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_unique_words_overall}", f"{self.html_stats.n_unique_words_overall}"
        )
        # Links Shared
        self.html_txt = self.html_txt.replace(
            "{n_links_u1}", f"{self.html_stats.n_links_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_links_u2}", f"{self.html_stats.n_links_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_links_overall}", f"{self.html_stats.n_links_overall}"
        )
        # Emojis Sent
        self.html_txt = self.html_txt.replace(
            "{n_emojis_u1}", f"{self.html_stats.n_emojis_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_emojis_u2}", f"{self.html_stats.n_emojis_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_emojis_overall}", f"{self.html_stats.n_emojis_overall}"
        )
        # Screen Touches
        self.html_txt = self.html_txt.replace(
            "{n_touches_u1}", f"{self.html_stats.n_touches_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_touches_u2}", f"{self.html_stats.n_touches_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_touches_overall}", f"{self.html_stats.n_touches_overall}"
        )
        # Days Texted (and %)
        self.html_txt = self.html_txt.replace(
            "{n_days_texted_u1}", f"{self.html_stats.n_days_texted_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_days_texted_u2}", f"{self.html_stats.n_days_texted_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{n_days_texted_overall}", f"{self.html_stats.n_days_texted_overall}"
        )
        # Most Used Word
        ...
        self.html_txt = self.html_txt.replace(
            "{word_overall}", f"{self.html_stats.word_overall}"
        )
        # Most Used Double Word
        self.html_txt = self.html_txt.replace(
            "{double_word_u1}", f"{self.html_stats.double_word_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{double_word_u2}", f"{self.html_stats.double_word_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{double_word_overall}", f"{self.html_stats.double_word_overall}"
        )
        # Most Used Triple Word
        self.html_txt = self.html_txt.replace(
            "{triple_word_u1}", f"{self.html_stats.triple_word_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{triple_word_u2}", f"{self.html_stats.triple_word_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{triple_word_overall}", f"{self.html_stats.triple_word_overall}"
        )
        # Most Used Emoji
        self.html_txt = self.html_txt.replace(
            "{emoji_u1}", f"{self.html_stats.emoji_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{emoji_u2}", f"{self.html_stats.emoji_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{emoji_overall}", f"{self.html_stats.emoji_overall}"
        )
        # First Message Date
        self.html_txt = self.html_txt.replace(
            "{first_msg_date_u1}", f"{self.html_stats.first_msg_date_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{first_msg_date_u2}", f"{self.html_stats.first_msg_date_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{first_msg_date_overall}", f"{self.html_stats.first_msg_date_overall}"
        )
        # Recent Message Date
        self.html_txt = self.html_txt.replace(
            "{recent_msg_date_u1}", f"{self.html_stats.recent_msg_date_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{recent_msg_date_u2}", f"{self.html_stats.recent_msg_date_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{recent_msg_date_overall}", f"{self.html_stats.recent_msg_date_overall}"
        )
        # Most Active Date
        self.html_txt = self.html_txt.replace(
            "{most_active_date_u1}", f"{self.html_stats.most_active_date_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{most_active_date_u2}", f"{self.html_stats.most_active_date_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{most_active_date_overall}", f"{self.html_stats.most_active_date_overall}"
        )
        # Least Active Date
        self.html_txt = self.html_txt.replace(
            "{least_active_date_u1}", f"{self.html_stats.least_active_date_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{least_active_date_u2}", f"{self.html_stats.least_active_date_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{least_active_date_overall}",
            f"{self.html_stats.least_active_date_overall}",
        )
        # Avg. Message/day
        self.html_txt = self.html_txt.replace(
            "{avg_msgs_u1}", f"{self.html_stats.avg_msgs_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{avg_msgs_u2}", f"{self.html_stats.avg_msgs_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{avg_msgs_overall}", f"{self.html_stats.avg_msgs_overall}"
        )
        # Avg. Emojis/day
        self.html_txt = self.html_txt.replace(
            "{avg_emojis_u1}", f"{self.html_stats.avg_emojis_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{avg_emojis_u2}", f"{self.html_stats.avg_emojis_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{avg_emojis_overall}", f"{self.html_stats.avg_emojis_overall}"
        )
        # Avg. Links/day
        self.html_txt = self.html_txt.replace(
            "{avg_links_u1}", f"{self.html_stats.avg_links_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{avg_links_u2}", f"{self.html_stats.avg_links_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{avg_links_overall}", f"{self.html_stats.avg_links_overall}"
        )
        # Avg. Words/Message
        self.html_txt = self.html_txt.replace(
            "{avg_words_u1}", f"{self.html_stats.avg_words_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{avg_words_u2}", f"{self.html_stats.avg_words_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{avg_words_overall}", f"{self.html_stats.avg_words_overall}"
        )
        # Avg. Letters/Message
        self.html_txt = self.html_txt.replace(
            "{avg_letters_u1}", f"{self.html_stats.avg_letters_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{avg_letters_u2}", f"{self.html_stats.avg_letters_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{avg_letters_overall}", f"{self.html_stats.avg_letters_overall}"
        )
        # Active Day - Date
        self.html_txt = self.html_txt.replace(
            "{active_date_u1}", f"{self.html_stats.active_date_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{active_date_u2}", f"{self.html_stats.active_date_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{active_date_overall}", f"{self.html_stats.active_date_overall}"
        )
        # Active Day - Messages
        self.html_txt = self.html_txt.replace(
            "{active_date_msgs_u1}", f"{self.html_stats.active_date_msgs_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{active_date_msgs_u2}", f"{self.html_stats.active_date_msgs_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{active_date_msgs_overall}", f"{self.html_stats.active_date_msgs_overall}"
        )
        # Active Day Words
        self.html_txt = self.html_txt.replace(
            "{active_date_words_u1}", f"{self.html_stats.active_date_words_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{active_date_words_u2}", f"{self.html_stats.active_date_words_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{active_date_words_overall}",
            f"{self.html_stats.active_date_words_overall}",
        )
        # Active Day Duration
        self.html_txt = self.html_txt.replace(
            "{active_date_duration_u1}", f"{self.html_stats.active_date_duration_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{active_date_duration_u2}", f"{self.html_stats.active_date_duration_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{active_date_duration_overall}",
            f"{self.html_stats.active_date_duration_overall}",
        )
        # Active Day Emojis
        self.html_txt = self.html_txt.replace(
            "{active_date_emojis_u1}", f"{self.html_stats.active_date_emojis_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{active_date_emojis_u2}", f"{self.html_stats.active_date_emojis_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{active_date_emojis_overall}",
            f"{self.html_stats.active_date_emojis_overall}",
        )
        # Longest Conversation Date
        self.html_txt = self.html_txt.replace(
            "{lc_date_u1}", f"{self.html_stats.lc_date_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{lc_date_u2}", f"{self.html_stats.lc_date_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{lc_date_overall}", f"{self.html_stats.lc_date_overall}"
        )
        # Longest Conversation Messages
        self.html_txt = self.html_txt.replace(
            "{lc_date_msgs_u1}", f"{self.html_stats.lc_date_msgs_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{lc_date_msgs_u2}", f"{self.html_stats.lc_date_msgs_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{lc_date_msgs_overall}", f"{self.html_stats.lc_date_msgs_overall}"
        )
        # Longest Conversation Words
        self.html_txt = self.html_txt.replace(
            "{lc_date_words_u1}", f"{self.html_stats.lc_date_words_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{lc_date_words_u2}", f"{self.html_stats.lc_date_words_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{lc_date_words_overall}", f"{self.html_stats.lc_date_words_overall}"
        )
        # Longest Conversation Duration
        self.html_txt = self.html_txt.replace(
            "{lc_date_duration_u1}", f"{self.html_stats.lc_date_duration_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{lc_date_duration_u2}", f"{self.html_stats.lc_date_duration_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{lc_date_duration_overall}", f"{self.html_stats.lc_date_duration_overall}"
        )
        # Longest Conversation Emojis
        self.html_txt = self.html_txt.replace(
            "{lc_date_emojis_u1}", f"{self.html_stats.lc_date_emojis_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{lc_date_emojis_u2}", f"{self.html_stats.lc_date_emojis_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{lc_date_emojis_overall}", f"{self.html_stats.lc_date_emojis_overall}"
        )
        # Response time stats
        self.html_txt = self.html_txt.replace(
            "{avg_response_u1}", f"{self.html_stats.avg_response_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{avg_response_u2}", f"{self.html_stats.avg_response_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{avg_response_overall}", f"{self.html_stats.avg_response_overall}"
        )
        # Top Sentiments
        self.html_txt = self.html_txt.replace(
            "{top_k_positive_u1}", f"{self.html_stats.top_k_positive_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{top_k_positive_u2}", f"{self.html_stats.top_k_positive_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{top_k_negative_u1}", f"{self.html_stats.top_k_negative_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{top_k_negative_u2}", f"{self.html_stats.top_k_negative_u2}"
        )
        # Emoji Ranking
        self.html_txt = self.html_txt.replace(
            "{emoji1_u1}", f"{self.html_stats.emoji1_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{emoji1_u2}", f"{self.html_stats.emoji1_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{emoji2_u1}", f"{self.html_stats.emoji2_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{emoji2_u2}", f"{self.html_stats.emoji2_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{emoji3_u1}", f"{self.html_stats.emoji3_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{emoji3_u2}", f"{self.html_stats.emoji3_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{emoji4_u1}", f"{self.html_stats.emoji4_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{emoji4_u2}", f"{self.html_stats.emoji4_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{emoji5_u1}", f"{self.html_stats.emoji5_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{emoji5_u2}", f"{self.html_stats.emoji5_u2}"
        )
        return self
    ...
    def populate_html_img(self):
        ...
        """
        self.html_txt = self.html_txt.replace(
            "{word_cloud_1words_u1}", f"{self.html_images.word_cloud_1words_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{word_cloud_1words_u2}", f"{self.html_images.word_cloud_1words_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{word_cloud_2words_u1}", f"{self.html_images.word_cloud_2words_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{word_cloud_2words_u2}", f"{self.html_images.word_cloud_2words_u2}"
        )
        self.html_txt = self.html_txt.replace(
            "{word_cloud_3words_u1}", f"{self.html_images.word_cloud_3words_u1}"
        )
        self.html_txt = self.html_txt.replace(
            "{word_cloud_3words_u2}", f"{self.html_images.word_cloud_3words_u2}"
        )

        self.html_txt = self.html_txt.replace(
            "{date_n_msgs}", f"{self.html_images.date_n_msgs}"
        )
        self.html_txt = self.html_txt.replace(
            "{date_n_emojis}", f"{self.html_images.date_n_emojis}"
        )
        self.html_txt = self.html_txt.replace(
            "{weekly_pattern}", f"{self.html_images.weekly_pattern}"
        )
        self.html_txt = self.html_txt.replace(
            "{hourly_pattern}", f"{self.html_images.hourly_pattern}"
        )
        self.html_txt = self.html_txt.replace(
            "{monthly_msg_pattern}", f"{self.html_images.monthly_msg_pattern}"
        )
        self.html_txt = self.html_txt.replace(
            "{monthly_word_pattern}", f"{self.html_images.monthly_word_pattern}"
        )
        self.html_txt = self.html_txt.replace(
            "{monthly_emoji_pattern}", f"{self.html_images.monthly_emoji_pattern}"
        )
        self.html_txt = self.html_txt.replace(
            "{monthly_response_pattern}", f"{self.html_images.monthly_response_pattern}"
        )
        self.html_txt = self.html_txt.replace(
            "{monthly_first_text_ct}", f"{self.html_images.monthly_first_text_ct}"
        )
        self.html_txt = self.html_txt.replace(
            "{monthly_avg_polarity}", f"{self.html_images.monthly_avg_polarity}"
        )
        self.html_txt = self.html_txt.replace(
            "{domain_counts}", f"{self.html_images.domain_counts}"
        )
        return self
